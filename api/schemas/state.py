...
class TrainingStateSaveResponse(StateSaveResponse):
    # 자동 수집 주기 반영(스케줄러 reschedule/pause)은 상태 저장과
...
class LatestStateResponse(BaseModel):
    ...
    # schema_version is stale) -- never a 404.
    training: dict[str, Any] | None
    analysis: dict[str, Any] | None
    alarms: dict[str, Any] | None
    # 앱 마운트 시 1번의 요청으로 알림 설정도 함께 복원한다
    # -- 설정 패널을 위한 별도 요청을 만들지 않는다.
    notifications: dict[str, Any]
    # 저장된 레코드 중 하나 이상이 더 이상 존재하지 않는
    ...
    # 복원 자체가 실패했으면(DB 손상, 조회 예외 등) true -- "저장된
