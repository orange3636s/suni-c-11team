...
# 자동화(주기 SQL 수율 예측 발송) 초기 등록 주기 -- "알림·자동화
...
def _log_storage_diagnostics() -> None:
    """기동 시 1회, 5개 저장 경로가 볼륨 안인지·쓰기 가능한지 남긴다.
...
# `asyncio.create_task(...)`의 반환값을 아무 데도 보관하지 않으면
...
# 첫 기동 스냅샷 부트스트랩은 자동 갱신 파이프라인(`run_refresh_pipeline`)을
# 그대로 재사용한다. 별도 파이프라인을 두지 않는다.
...
def _check_dataset_file_integrity() -> None:
    """`datasets` 레코드는 남아 있는데 업로드 파일이 볼륨 밖에서
...
class BundledTrainingDataMissingError(RuntimeError):
    """부트스트랩/복구 재학습이 내장 학습 데이터(data/bundled/train.CSV)
    자체를 찾지 못해 실패한 경우 -- 다음 요청이 재시도해도 파일이
    저절로 생기지 않으므로, 다른 예외(일시적 실패)와 구분해 배너가
    재시도 없는 구체적 안내를 보여줄 수 있게 한다."""


# `bootstrap_status.reason`에 싣는 코드 -- 프런트 BootstrapStatusBanner가
...
def _has_usable_champion(store: RuntimeStore) -> bool:
    """`store.active_model()`은 DB의 `model_slots` 행이 있는지만
    ...
    로드에 실패하면(파일이 실제로 없음) 레지스트리에 남은 포인터를
    그대로 두지 않는다 -- 이 함수가 매 요청(런타임 복구 훅)마다
...
async def _train_bootstrap_champion() -> None:
    """`run_refresh_pipeline`은 학습을 트리거하지 않는다(학습 경로는 "모델
    학습" 팝업의 수동 업로드 하나뿐이다). 따라서 챔피언이 전혀 없는
    환경(신규 배포, 또는 챔피언 포인터는 있으나 아티팩트가 없는 경우)에서는
    콜드 스타트가 직접 학습해야 하며, 파이프라인이 학습 Job을 제출하길
    기다리면 부트스트랩이 영원히 끝나지 않는다. 수동 업로드와 같은 코드
    경로(`train_model`)를 내장 train.CSV로 그대로 재사용해 별도 학습
    파이프라인을 두지 않는다."""
...
async def _run_bootstrap(store: RuntimeStore) -> None:
    ...
    try:
        ...
        # 다른 예외와 달리 reason을 남긴다 -- BootstrapStatusBanner가
        ...
        # reason 없는 "failed"는 일시적 실패로 취급한다 -- 이
        # 상태 자체가 다음 재시도를 막지 않는다(런타임 복구 훅은 오직
...
async def _bootstrap_snapshot_background() -> None:
    ...
    `_spawn_background_task`로 fire-and-forget한다(`_warm_bundled_dataset_cache`와
    같은 이유 -- 헬스체크가 실패하면 배포가 롤백된다). 수동 override로
    활성화된 평가 데이터셋이 있으면 그 자체가 사용자의 최근 작업이므로
    내장 데이터로 부트스트랩을 돌려 덮어쓰지 않는다.

    `has_valid_snapshot()`은 스냅샷 행의 스키마 버전과, 그 안에 기록된
    model_id가 현재 `model_slots.active_model_id`와 같은지만 본다 -- 그
    모델 파일이 `models/`에 실제로 있는지는 확인하지 않는다. 스냅샷이
    이미 저장된 뒤 `models/`가 비워지고(예: 배포 볼륨 유실) 서버가
    재시작되면 이 게이트만으로는 "유효한 스냅샷 있음"으로 잘못 판단해
    `_run_bootstrap`을 건너뛰고, 그러면 런타임 요청이 영원히 409로 막힌 채
    아무도 재학습을 트리거하지 않는다. `_has_usable_champion`으로 파일
    존재까지 확인된 경우에만 건너뛴다."""
...
def ensure_usable_champion(store: RuntimeStore) -> bool:
    """런타임 요청(`api/routes/analysis.py::_hydrated_targets_or_409`
    ...
    이 함수는 부트스트랩 1회가 아니라 409가 발생하는 요청마다
    ...
    # 호출부(현재 요청을 처리 중인 FastAPI 스레드풀 워커)에는
...
async def lifespan(app: FastAPI):
    # 방금 새로 뜬 프로세스에는 정의상 진행 중인 "분석 시작"
    ...
    try:
        ...
        # last_run도 같은 이유로 부팅 시 정리한다 -- "running"으로 고아가
        # 됐으면 failed로 바꾼다.
    ...
    # 첫 기동 스냅샷 부트스트랩 -- 유효 스냅샷이 있으면 내부에서
    ...
    # Telegram 알림 연동 -- 봇 토큰이 설정된 경우에만 long-polling
    ...
    if settings.telegram_bot_token:
        ...
        # 토큰만 있고 username이 없으면 폴링은 도는데 화면에는 봇
    ...
    # 자동 알림 발송 경로는 아래 automation_yield_dispatch(Refresh Time
    # 주기) 잡 하나뿐이며, 매 주기 조건 없이 발송을 시도한다.
    scheduler = AsyncIOScheduler()
    # notify_sent_log 정리 -- 발송 잡과 겹치지 않도록 별도 id·시각으로
    ...
    # 알림 기록 보관 정책(최근 100건/30일) 보조 정리 -- 발송 잡과
    ...
    # 자동화(주기 SQL 수율 예측 발송) -- "알림·자동화 설정" 팝업의
    # 전용 슬롯(automation:settings)에서 켜짐 여부·주기를 읽는다.
    # 자동화는 수율 예측만 계산한다(화면 재계산은 하지 않는다).
...
# GET /api/state/latest bundles 3 tabs' worth of results in one response and
# easily exceeds 100 KB. Compression applies to every response over 1KB, not
# just that endpoint, since it's free insurance for every other JSON payload.
