"""웨이퍼별 신뢰도 점수(0~100)와 y1~y5 셀 색상. 알림 목록이 y 합산 기준
순위를 쓰기 때문에 필요하다 -- 실측이 하나도 없는 wafer는 다섯 모드가
전부 모델 추정값이고, 핵심 인자마저 미계측이면 사실상 평균값의 합이다.
그런 y가 "수율 낮은 10개"에 들어가면 근거 없는 알림이 된다.

산식: 모드 신뢰_k = 1.0(실측) | 파레토 기여율(예측+인자 계측) |
...
def primary_factors_by_target(train_df: pd.DataFrame, schema: Schema, targets: tuple[str, ...] = FAIL_RATE_TARGETS) -> dict[str, PrimaryFactor | None]:
    """타깃별 1위(최대 Adjusted R²) 인자 -- train.CSV 기준으로 한 번만
    계산해 재사용한다(신뢰도 점수와 셀 색상 둘 다 이 값을 쓴다).
    후보가 아예 없는 타깃은
