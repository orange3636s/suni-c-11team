...
예측 구간은 홀드아웃 잔차 기반 conformal 방식으로 산출하며 목표 포함률 90%를 보장한다. 웨이퍼 한 장 단위 여유는 약 ±5.5%p이고, 집계(전체·랏 평균) 여유는 별도 산출하며 약 ±0.2%p로 훨씬 좁다 — 평균의 불확실성이 개별값보다 작기 때문이며, 모델 결함이 아니라 데이터의 설명력 한계(R² 약 0.24)를 반영한 결과다.
