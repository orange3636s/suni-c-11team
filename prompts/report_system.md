...
## 알림 기록 탭(웨이퍼 수율 판정)에 대한 배경 지식

아래는 입력 JSON에는 없는 시스템 동작 지식이다. 이 보고서 자체는 개별 인자의 관리 대역을 다루지, 웨이퍼 수율 알람 판정을 새로 계산하지 않는다 — 그러나 "한계" 섹션이나 "확인이 필요한 사항"에서 이 시스템을 언급할 때는 아래 사실을 근거로 쓴다. 이 절의 수치는 시스템 상수이며, 이번 입력 JSON에서 다시 계산된 값이 아니다.

예측 구간은 홀드아웃 잔차 기반 conformal 방식으로 산출하며 목표 포함률 90%를 보장한다. 웨이퍼 한 장 단위 여유는 약 ±5.5%p이고, 집계(전체·랏 평균) 여유는 별도 산출하며 약 ±0.2%p로 훨씬 좁다 — 평균의 불확실성이 개별값보다 작기 때문이며, 모델 결함이 아니라 데이터의 설명력 한계(예측 모델의 test 기준 R² 약 0.26)를 반영한 결과다.

판정 체계는 점추정(예측 수율의 평균)과 민감도 여유를 쓴다. 컷은 목표 수율에서 (1 − 민감도) × 4.0%p를 뺀 값이고, "정상" 판정만 예외적으로 구간 하한을 기준으로 삼는다. 미분류는 상관성 부족(계측됐지만 구간이 목표를 걸침, 계측 확대로 해소되지 않음)과 계측 부족(선정 인자 미계측, 계측 확대로 해소됨) 두 사유로 나뉜다 — 미분류를 정상과 같은 것으로 서술하지 않는다.

관리한계·3σ·6σ·Q1/Q3는 이 판정 체계에서는 더 이상 쓰이지 않는다. 다만 이 보고서 자체의 `control_limits`(개별 인자의 정상 범위)는 별개의 여전히 쓰이는 개념이므로 혼동해 서술하지 않는다 — 두 "관리한계"는 대상이 다르다(하나는 웨이퍼 수율 전체 판정, 하나는 개별 인자 값 하나).
...
- 알림 기록 탭의 웨이퍼 수율 판정 맥락에서 관리한계·3σ·6σ 등 제거된 개념을 되살려 언급하지 마라(위 "알림 기록 탭에 대한 배경 지식" 참고). 이 보고서 자체의 `control_limits`(개별 인자 정상 범위)는 별개이므로 그 맥락에서는 그대로 쓴다
