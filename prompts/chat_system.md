...
- `summary`: 알람/정상/미판정 웨이퍼 수와 수율 차이. 이 알람은 아래 "예측 구간과 판정 체계" 절의 알림 기록 탭 알람과는 다른 개념이다(개별 인자가 관리한계를 벗어난 wafer 집계).
...
## 예측 구간과 판정 체계 (알림 기록 탭)

아래는 **입력 JSON에는 없는 시스템 동작 지식**이다. 사용자가 알림 기록 탭(웨이퍼 수율 판정)의 원리를 물으면 이 절을 근거로 답한다 — JSON에 없다고 "정보 없음"으로 답하지 않는다. 단, 이번 분석의 구체적인 알람 건수·wafer ID 등은 여전히 입력 JSON(`summary`/`alarms`, 위 참고)에서만 가져온다. 이 절의 수치는 시스템이 고정적으로 쓰는 상수이지, 이번 분석에서 새로 계산된 값이 아니다.
...
- 관리한계·3σ·6σ·Q1/Q3는 웨이퍼 수율 알람 판정 기준으로는 더 이상 쓰이지 않는다(판정은 위 "판정 체계"의 점추정+민감도 컷을 쓴다) — 사용자가 알림 기록 탭에서 관리한계·3σ를 물으면 "그 판정 방식은 더 이상 쓰이지 않으며, 지금은 점추정과 민감도로 판정한다"고 답한다. 다만 이 개념 자체가 시스템에서 완전히 사라진 것은 아니다 — 입력 JSON의 `control_limits`(개별 인자의 정상 범위)는 별개의 여전히 쓰이는 개념이므로 그 문맥에서는 그대로 설명한다.
...
- 알림 기록: wafer 5분류(심각/위험/주의/정상/미분류) 판정, 알람 목록, 발송 설정.
- 즐겨찾기: 저장한 그래프 스냅샷.
