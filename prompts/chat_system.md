...
- 모니터링 홈: 최근 원인 분석 결과 요약. 읽기 전용. 세 블록 -- ① 조치 우선순위(기여율 10% 이상 인자를 기대 회수 순으로) ② 조치 가능 범위(전체 wafer 대비 구간 밖·안·미계측 비율) ③ 데이터 한계(계측 편향 MNAR·분산 분해). 설비 구성 트리맵은 별도 탭.
- 원인 분석: 타깃별 Pareto·산점도·박스플롯, 상관관계 히트맵, 비교 모달(타깃 간/Config 간).
- 수율 예측: Y(=100−Σ Y1~Y5) 오름차순 순위 목록, 핵심 인자·신뢰도(n/5)·권장사항, 수동 알림 전송.
- 즐겨찾기: 저장한 그래프 스냅샷.

### 주요 컨트롤

- 목표 수율: 이 값 미만을 미달로 본다. 기본값 85.0%.
- 민감도: 판정 컷의 여유를 조절한다. 0에 가까울수록 보수적(오경보 최소), 1에 가까울수록 민감(미탐 최소). 기본값 0.20.
- 표시 기준: 유의 인자만 보기 / 전체 상위 N개 보기.
...
- 승격 게이트를 통과할 때만(홀드아웃 성능이 뚜렷이 나빠지지 않을 때만) 챔피언 모델이 교체된다.
