...
//      had -- a narrower payload (no scatter points) than a
...
export type TrainingState = {
  ...
  // 모델 학습 팝업의 SQL 연결 정보(비밀번호 제외)·Refresh
...
export type AnalysisState = {
  ...
  // 있다. `fmea`가 null인데 `fmeaError`도 null이면 이 필드가 채워지기
  // 전 형식으로 저장된 레코드라는 뜻 -- 다시 분석하면 채워진다.
  // `fmeaError`가 있으면 계산이 실패한 것이다.
  fmea?: FmeaTablePayload | null;
  fmeaError?: string | null;
  // 모니터링 홈 블록①·② -- fmea와 같은 두 경로·같은 null 규칙을
  ...
  // 상관관계 히트맵 캐시 -- 탭을 오가는 동안(CorrelationHeatmap이
...
// 모니터링 홈이 마지막으로 렌더한 결과를 탭 이동에도 살아남는
...
export type MonitoringHomeState = {
  ...
  // 전부 `snapshot.fmea`/`snapshot.actionPriority`에 있으므로, 이 캐시를
  // 채우는 데 별도 API 호출이 필요하지 않다.
...
type AnalysisStateValue = {
  ...
  // skeleton" vs "nothing saved, show the empty state".
  hydrated: boolean;
  ...
  // 저장된 학습/분석/알람 결과 중 하나 이상이 이미 없는 데이터셋을
  // 가리켜 서버가 통째로 버렸다는 신호. true면 "선택했던 데이터셋이
  // 없어 train으로 전환했습니다" 안내를 보여준다.
  datasetFallbackNotice: boolean;
  // 이전 결과 복원(GET /api/state/latest) 자체가 실패했다는 신호
  ...
  // state/latest가 마운트 시 1번에 실어 온 알림 설정.
  ...
  // 자동 갱신 파이프라인이 저장한 단일 스냅샷 -- 모니터링 홈은 이 값만
  // 읽고 자기 스스로 API를 부르지 않는다(탭이 개별로 fetch하지
  // 않는다). null은 "아직 한 번도 갱신되지 않음"(최초 기동)과
  // "복원 실패"를 구분하지 않는다 -- 둘 다 화면은 "첫 갱신 대기 중"으로
  // 같게 취급한다.
  snapshot: RefreshSnapshot | null;
  // 지금 schema_version과 맞지 않는 스냅샷이 남아 있다는 신호 -- true면
  ...
  // 첫 기동 부트스트랩 진행 상태. null이면 부트스트랩이 아직 한 번도
  // 보고되지 않은 것(서버가 보고하지 않거나, 메타를 아직 못 받아옴)이고
  // 화면은 이를 "부트스트랩 없음"과 동일하게 취급한다.
  bootstrapStatus: BootstrapStatus | null;
  // 실패 배너의 "다시 시도"용 -- 폴링 타이머를 기다리지 않고 즉시
  // meta/snapshot을 다시 확인한다.
  refreshSnapshotNow: () => void;
  // "모델 분석"(부트스트랩·학습 후 자동 복구 포함)이 지금 실행 중인지.
  refreshRunning: boolean;
  // 네 화면이 공유하는 "분석 시작" 진행 표시 -- 실행 중이 아니면 null.
  analysisProgress: AnalysisProgress | null;
  // 등록된 활성 분석 데이터가 업로드/DB 불러오기로 설정된 것이면
  // 그 정보. null이면 내장(기본) 데이터.
  manualEvalOverride: ManualEvalOverride | null;
  // "분석 시작"의 최근 실행 결과 -- 백그라운드 실행이 조용히 실패했을 때
  // 원인을 보여주는 데 쓴다.
  lastRun: LastRunStatus | null;
};

// 부트스트랩/주기 자동 갱신이 채운 스냅샷을, 한 번도 저장된 적
// 없는(=클릭한 적 없는) analysis/alarms의 대체 데이터로 쓴다. 화면들이
// 이미 알고 있는 "복원됐지만 아직 좌표/원시 예측치는 없는" 모양
// (pointsComplete:false, data:null)으로 채우므로, 각 화면의
...
export default function AnalysisStateProvider({ children }: { children: ReactNode }) {
  ...
  // 복원 실패(서버가 degraded:true를 보고했거나, 이 요청 자체가
  ...
  // 다음 폴링 간격을 고르는 재귀 setTimeout이 매 실행 시점의 최신
  ...
  // refreshRunning도 같은 이유로 ref가 필요하다 -- scheduleNext가
  ...
  function hydrate() {
    ...
    getLatestState()
      .then((state) => {
        ...
        if (state.analysis) {
          ...
          if (isAnalysisSnapshotUsable(state.analysis, state.training?.created_at ?? null)) {
            // Server-persisted analysis payload is Pareto-only (snapshot
            ...
            setAnalysis({
              ...
              // 저장 시점(POST /api/state/analysis)에 서버가 채워
              // 넣으므로 그대로 복원한다 -- 이 값이 undefined인 것은 이
              // 필드가 채워지기 전 형식으로 저장된 레코드뿐이다.
              // actionPriority도 같은 규칙(`_with_action_priority`).
      ...
      .catch(() => {
        // 복원 실패가 앱을 막으면 안 된다 -- start empty. 하지만
  ...
  // 재시도 버튼이 부른다 -- 마운트 가드(hydrationStarted)를 우회해
  ...
  // 스냅샷 전체를 받아 상태를 갱신한다. `announce`가 true면(폴링이
  ...
  const applySnapshot = useCallback((full: Awaited<ReturnType<typeof getSnapshot>>, announce: boolean) => {
    ...
    // analysis/alarms가 아직 아무것도 없으면(저장된 결과도, 이전에
  ...
  // 실패 배너의 "다시 시도"가 부른다 -- 폴링 타이머를 기다리지 않고
  ...
  useEffect(() => {
    ...
    // 고정 setInterval 대신 재귀 setTimeout을 쓴다 -- 매 회차가
    ...
    // 마운트 즉시 한 번 확인해야 한다 -- 첫 폴링을 기다리면 최대 60초
    // 동안 배너가 뜨지 않는다.
    void checkForUpdate();
    scheduleNext();
