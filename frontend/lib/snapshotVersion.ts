...
// 변경이 그 첫 사례였고, 작업 지시서 WE(FMEA 표를 RPN 상위 N개에서
// 타깃별 기여율 20% 이상으로 재선정, S·O·D·RPN 필드 제거)가 3으로 올린
// 계기다. 백엔드의 봉투 schema_version(src/runtime/app_state.py
// STATE_SCHEMA_VERSION)과는 의도적으로 별개다: 그쪽이 버전 불일치를
// 서버에서 필터링해 버리면(record -> null) 프론트는 "분석을 실행한 적이
// 없다"와 "낡은 스냅샷이라 폐기됐다"를 구분할 수 없어 조용히 빈 화면이
// 된다. 이 버전은 payload 안에 실려 그대로 내려오므로, 프론트가 직접
// 비교해 폐기 사유를 안내할 수 있다.
