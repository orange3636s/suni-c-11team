...
export type MonitoringSnapshot = {
  ...
  dataset: string | null;
  // 블록③(데이터 한계) 원천 -- MNAR·분산 분해만 담는다(MA-3, 행별 FMEA
  // 표는 삭제). 자동 갱신 스냅샷과 수동 "다시 분석" 저장 둘 다 백엔드가
  // 채워 보낸다(JA-1). `fmea`가 null인데 `fmeaError`도 null이면 JA-1
  // 배포 이전에 저장된 옛 레코드 -- 다시 분석하면 채워진다. `fmeaError`가
  // 있으면 계산이 실패한 것이다.
 ...
 * 감싼다. 블록①②③에 필요한 값은 모두 백엔드가 이미 다 계산해
 * analysis.fmea/analysis.actionPriority에 실어 보내므로(지시서
 * IA-5/MF-2) 여기서 추가로 조회하지 않는다 -- 화면은 읽어서 그리기만
 * 한다.
 */
export async function buildMonitoringSnapshot(
  ...
  // 계속 다룬다(기존 렌더 코드를 건드리지 않기 위해) -- AnalysisStateProvider의
