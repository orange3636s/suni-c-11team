      ...
      <main className="mainContent dashboardPage">
        ...
        <section className="dashboardSplit">
          ...
          <aside className="resultCard dashboardDetail">
            {!selected ? <p>알람을 선택하면 상세 정보가 표시됩니다.</p> : <><span className="sectionLabel">Alert Detail</span><h2>{selected.lot_wafer_id}</h2><dl>{Object.entries({"Analysis ID":selected.analysis_id,"Model ID":selected.model_id,"Direct Y":display(selected.direct_y,"%"),"Derived Y":display(selected.derived_y,"%"),"Top Feature":selected.top_feature ?? "-","Top Step":selected.top_step ?? "-","Equipment":selected.top_equipment ?? "-","Acknowledged":selected.acknowledged_at ?? "-","Resolved":selected.resolved_at ?? "-"}).map(([label,value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl><div className="dashboardActions"><button className="button secondary" disabled={selected.status !== "New"} onClick={() => void changeStatus("Acknowledged")}>Acknowledge</button><button className="button primary" disabled={selected.status === "Resolved"} onClick={() => void changeStatus("Resolved")}>Resolve</button></div></>}
          </aside>
        </section>
